...
{
    string nombre = "Settings/Constructs/ConstructImages/Name";
    ...
    public class TilemapToPng : MonoBehaviour
{
    ...
    [HideInInspector] public Texture2D ImagenLista;
    public Sprite fallbackTransparentSprite;
        ...
        for (int x = 0; x < tm.size.x; x++) //Hallamos el punto menor y mayor
        {
            for (int y = 0; y < tm.size.y; y++)
            {
                ...
                if (tm.GetSprite(pos) == null)
                {
                    ...
                    SpriteCualquiera = fallbackTransparentSprite;
                    //TODO if empty it should use a transparent sprite
                }
                else
                {
